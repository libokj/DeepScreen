...
DeepScreen is a PyTorch-based DTI prediction toolkit with a unified data
pipeline and a modular framework. It aims to provide performant DTI prediction and
fulfil non-functional design requirements that cater for user and developer needs.
...
Train a DTI model with default configuration:

```bash
# train on CPU
python deepscreen/train.py model=dti datamodule=dti trainer=cpu

# train on GPU
python deepscreen/train.py model=dti datamodule=dti trainer=gpu
```

Train model with chosen experiment configuration from [configs/experiment/](configs/experiment/):

```bash
python deepscreen/train.py experiment=experiment_name.yaml
```

Override any parameter from command line:

```bash
# change maximum training epochs to 20 and use DAVIS dataset
python deepscreen/train.py model=dti datamodule=dti trainer.max_epochs=20 datamodule.dataset_name=davis
```
...
<details>
<summary><b>Train on CPU and GPU</b></summary>
...
```bash
# this will run hyperparameter search defined in `configs/hparams_search/dti_optuna.yaml`
# over chosen experiment config
python train.py -m hparams_search=dti_optuna experiment=example
```

> **Note**: Using [Optuna Sweeper](https://hydra.cc/docs/next/plugins/optuna_sweeper) doesn't require you to add any boilerplate to your code, everything is defined in a [single config file](configs/hparams_search/dti_optuna.yaml).
...
```bash
python train.py tags=["dti","experiment_X"]
```
...
[2022-07-11 15:40:09,358][deepscreen.utils.utils][INFO] - Enforcing tags! <cfg.extras.enforce_tags=True>
[2022-07-11 15:40:09,359][deepscreen.utils.rich_utils][WARNING] - No tags provided in config. Prompting user to input tags...
...
> **Note**: Due to GitHub file size limit, the only available dataset in the public version of DeepScreen is `DAVIS`. 
> A solution for hosting benchmark datasets will be soon proposed.

## Documentation

> **Note**: Detailed documentation for the public version of DeepScreen is under active development.

## Graphical user interface

> **Note**: GUI for the public version of DeepScreen is under active development.